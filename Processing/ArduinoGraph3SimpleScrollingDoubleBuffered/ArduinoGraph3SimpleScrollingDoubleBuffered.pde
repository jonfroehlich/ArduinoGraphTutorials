...
void serialEvent (Serial myPort) { 
  ...
  if (inString != null) {
    ...
    // Slide everything down in the array while also drawing to the
    // graphics buffer
    for (int i = 0; i < _yPixelValues.length - 1; i++) {
      int nextYPixelVal = _yPixelValues[i+1];
      _yPixelValues[i] = nextYPixelVal;
      _pg.line(i, height, i, height - nextYPixelVal);
    }

    _yPixelValues[_yPixelValues.length - 1] = yPixelValue;
    _pg.line(_yPixelValues.length - 1, height, _yPixelValues.length - 1, height - yPixelValue);
    _pg.endDraw();
